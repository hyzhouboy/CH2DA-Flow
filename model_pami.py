...
from tool import Logger, Project3D, BackprojectDepth, gemerate_haze, EMA, warp_error
from loss import sequence_loss, disp_pyramid_loss, motion_consis_loss, cross_domain_consis_loss, self_supervised_loss, loss_KL_div, info_nce_loss

from utils.utils import transformation_from_parameters, disp_to_depth, filter_base_params, filter_specific_params
from PIL import Image
from utils import transforms
from geometry import compute_flow_with_depth_pose
...
class Model(object):
    def __init__(self, args):
        ...
        # batch_size = 4
        self.K = torch.cat([self.K_, self.K_, self.K_, self.K_], 0).cuda()
        
        self.inv_K_ = torch.from_numpy(inv_K).unsqueeze(dim=0)
        # batch_size = 2
        # self.inv_K = torch.cat([self.inv_K_, self.inv_K_], 0).cuda()
        # batch_size = 4
        self.inv_K = torch.cat([self.inv_K_, self.inv_K_, self.inv_K_, self.inv_K_], 0).cuda()
        # print()
        ...
        while should_keep_training:
            for i_batch, data_blob in enumerate(train_loader):
                ...
                with torch.no_grad():
                    ...
                    # 3D 几何投影
                    disp_ = (depth_predictions[-1]*256.0)
                    depth = 1. / disp_ * fx * baseline
                    # 新版本，暂时不用
                    # compute_flow_with_depth_pose(1./depth.squeeze(1), self.K, extrinsics_rel=pred_poses)
                ...
                # flow network update
                flow_loss = flow_loss + 0.3 * geo_motion_loss
    ...
    def stage_4_train(self):
        ...
        while should_keep_training:
            for i_batch, data_blob in enumerate(train_loader):
                ...
                # flow network update
                flow_loss = flow_loss + 0.5 * flow_DA_loss
    ...
    # dataset: rain-kitti and clean images
    ...
    def stage_5_train(self):
        ...
        print("Parameter Count: %d" % count_parameters(model_flow_clean))
        # raft-rain
        model_flow_rain = nn.DataParallel(RAFT(self.args), device_ids=self.args.gpus)
        print("Parameter Count: %d" % count_parameters(model_flow_rain))
        ...
        if self.args.restore_flow_synimg_rain_ckpt is not None:
            model_flow_rain.load_state_dict(torch.load(self.args.restore_flow_synimg_rain_ckpt), strict=False)
             

        model_flow_clean.cuda()
        model_flow_clean.eval()

        model_flow_rain.cuda()
        model_flow_rain.train()


        if self.args.stage != 'chairs':
            model_flow_rain.module.freeze_bn()

        # training process
        flow_optimizer = optim.AdamW(model_flow_rain.parameters(), lr=self.args.lr, weight_decay=self.args.wdecay, eps=self.args.epsilon)
        flow_scheduler = optim.lr_scheduler.OneCycleLR(flow_optimizer, self.args.lr, self.args.num_steps+100,
            pct_start=0.05, cycle_momentum=False, anneal_strategy='linear')

        total_steps = 0
        flow_scaler = GradScaler(enabled=self.args.mixed_precision)
        logger = Logger('checkpoints/', model_flow_rain, flow_scheduler)
        ...
        while should_keep_training:
            for i_batch, data_blob in enumerate(train_loader):
                ...
                image1_clean = data_blob['clean_1'].cuda()
                image2_clean = data_blob['clean_2'].cuda()
                image1_rain = data_blob['rain_1'].cuda()
                image2_rain = data_blob['rain_2'].cuda()
                flow = data_blob['flow'].cuda()
                valid = data_blob['valid'].cuda()
                # image1, image2, flow, valid, disp  = [x.cuda() for x in data_blob]
                ...
                if add_noise:
                    ...
                    image1_clean = (image1_clean + stdv * torch.randn(*image1_clean.shape).cuda()).clamp(0.0, 255.0)
                    image2_clean = (image2_clean + stdv * torch.randn(*image2_clean.shape).cuda()).clamp(0.0, 255.0)
                    image1_rain = (image1_rain + stdv * torch.randn(*image1_rain.shape).cuda()).clamp(0.0, 255.0)
                    image2_rain = (image2_rain + stdv * torch.randn(*image2_rain.shape).cuda()).clamp(0.0, 255.0)
                
                with torch.no_grad():
                    _, flow_clean_predictions = model_flow_clean(image1_clean, image2_clean, iters=20, test_mode=True)
               
                # 计算浓雾天图像光流
                _, flow_rain_predictions = model_flow_rain(image1_rain, image2_rain, iters=self.args.iters)     
                
                flow_loss, flow_metrics = sequence_loss(flow_rain_predictions, flow, valid, self.args.gamma)
                flow_DA_loss, flow_DA_metrics = cross_domain_consis_loss(flow_rain_predictions[-1], flow_clean_predictions)
                
                # contrastive learning loss--补充这里
                contra_loss = 0.0
                if self.args.use_contra_loss:
                    warp_error_clean = warp_error(image1_clean, image2_clean, flow_clean_predictions[-1])
                    warp_error_rain = warp_error(image1_rain, image2_rain, flow_rain_predictions[-1])

                    # normalize the warp error
                    # warp_error_clean = (warp_error_clean - torch.mean(warp_error_clean)) / (torch.std(warp_error_clean) + 1e-6)
                    warp_error_clean = 2 * (warp_error_clean / 255.0) - 1.0
                    warp_error_rain = 2 * (warp_error_rain / 255.0) - 1.0

                    # feed into f-net
                    f_clean, _ = model_flow_rain.module.f_net([warp_error_clean, warp_error_clean])
                    f_rain, _ = model_flow_rain.module.f_net([warp_error_rain, warp_error_rain])

                    z_clean = F.adaptive_avg_pool2d(f_clean, 1).flatten(1)
                    z_rain = F.adaptive_avg_pool2d(f_rain, 1).flatten(1) 
                    
                    contra_loss = info_nce_loss(z_rain, z_clean, temperature=0.1)
                



                # flow network update
                flow_loss = flow_loss + 0.5 * flow_DA_loss + 0.1 * contra_loss
                
                flow_scaler.scale(flow_loss).backward()
                flow_scaler.unscale_(flow_optimizer)                
                torch.nn.utils.clip_grad_norm_(model_flow_rain.parameters(), self.args.clip)
                ...
                if total_steps % VAL_SUMMARY_FREQ == 0:
                    img_summary = dict()
                    img_summary['clean_1'] = image1_clean
                    img_summary['rain_1'] = image1_rain
                    img_summary['gt_flow'] = flow
                    img_summary['pred_flow'] = flow_rain_predictions[-1]
                    logger.save_image('train', img_summary)

                    
                if total_steps % VAL_FREQ == 0:
                    FLOW_PATH = 'checkpoints/%d_%s.pth' % (total_steps+1, 'raft_synrain')
                    torch.save(model_flow_rain.state_dict(), FLOW_PATH)
        ...
        logger.close()
        FLOW_PATH = 'checkpoints/%s.pth' % 'raft_synrain'
        torch.save(model_flow_rain.state_dict(), FLOW_PATH)
    ...
    def stage_7_train(self):
        ...
        while should_keep_training:
            for i_batch, data_blob in enumerate(train_loader):
                ...
                if total_steps % VAL_FREQ == 0:
                    SYN_PATH = 'checkpoints/%d_%s.pth' % (total_steps+1, 'kpa_raft_synfog')
                    torch.save(model_flow_syn.state_dict(), SYN_PATH)
                
                if total_steps % VAL_FREQ == 0:
                    # 更新EMA策略更新权重保存模型
                    model_ema.apply_shadow()
                    REAL_PATH = 'checkpoints/%d_%s.pth' % (total_steps+1, 'kpa_raft_realfog')
                    torch.save(model_flow_real.state_dict(), REAL_PATH)
                    model_ema.restore()
        ...
        logger.close()
        SYN_PATH = 'checkpoints/%s.pth' % 'kpa_raft_synfog'
        torch.save(model_flow_syn.state_dict(), SYN_PATH)

        model_ema.apply_shadow()
        REAL_PATH = 'checkpoints/%s.pth' % 'kpa_raft_realfog'
        torch.save(model_flow_real.state_dict(), REAL_PATH)
