...
class CrossDomainFlowDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img1_rain = frame_utils.read_gen(self.image_list[index][4])
        img2_rain = frame_utils.read_gen(self.image_list[index][5])
        ...
        img1_rain = np.array(img1_rain).astype(np.uint8)
        img2_rain = np.array(img2_rain).astype(np.uint8)
        ...
        if len(img1_clean.shape) == 2:
            ...
            img1_rain = np.tile(img1_rain[...,None], (1, 1, 3))
            img2_rain = np.tile(img2_rain[...,None], (1, 1, 3))
        else:
            img1_clean = img1_clean[..., :3]
            img2_clean = img2_clean[..., :3]
            img1_foggy = img1_foggy[..., :3]
            img2_foggy = img2_foggy[..., :3]
            img1_rain = img1_rain[..., :3]
            img2_rain = img2_rain[..., :3]

        if self.augmentor is not None:
            if self.sparse:
                img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid = self.augmentor(img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid)
        ...
        img1_rain = torch.from_numpy(img1_rain).permute(2, 0, 1).float()
        img2_rain = torch.from_numpy(img2_rain).permute(2, 0, 1).float()
        flow = torch.from_numpy(flow).permute(2, 0, 1).float()
        ...
        sample['rain_1'] = img1_rain
        sample['rain_2'] = img2_rain
...
class KITTI_Synthetic(CrossDomainFlowDataset):
    def __init__(self, aug_params=None, split='training', root='datasets/KITTI'):
        super(KITTI_Synthetic, self).__init__(aug_params, sparse=True)
        root = osp.join(root, split)

        images1_clean = sorted(glob(osp.join(root, 'image_2/*_10.png')))
        images2_clean = sorted(glob(osp.join(root, 'image_2/*_11.png')))

        images1_foggy = sorted(glob(osp.join(root, 'image_fog/*_10.png')))
        images2_foggy = sorted(glob(osp.join(root, 'image_fog/*_11.png')))

        images1_rain = sorted(glob(osp.join(root, 'image_rain/*_10.png')))
        images2_rain = sorted(glob(osp.join(root, 'image_rain/*_11.png')))

        for img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain in zip(images1_clean, images2_clean, images1_foggy, images2_foggy, images1_rain, images2_rain):
            frame_id = img1_clean.split('/')[-1]
            self.extra_info += [ [frame_id] ]
            self.image_list += [ [img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain] ]
...
def fetch_dataloader(args):
    ...
    if args.stage == 'chairs':
        ...
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.4, 'do_flip': False}
        train_dataset = KITTI_Synthetic(aug_params, split='training', root=args.data_dir)
    elif args.stage == 'kitti_rain':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.4, 'do_flip': False}
        train_dataset = KITTI_Synthetic(aug_params, split='training', root=args.data_dir)
