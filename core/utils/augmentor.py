...
class CrossDomainSparseFlowAugmentor:
    ...
    def spatial_transform(self, img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid):
        # randomly sample scale
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            img1_rain = cv2.resize(img1_rain, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2_rain = cv2.resize(img2_rain, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        ...
        if self.do_flip:
            if np.random.rand() < 0.5: # h-flip
                ...
                img1_rain = img1_rain[:, ::-1]
                img2_rain = img2_rain[:, ::-1]
                flow = flow[:, ::-1] * [-1.0, 1.0]
                valid = valid[:, ::-1]
        ...
        img1_rain = img1_rain[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img2_rain = img2_rain[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        flow = flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        valid = valid[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
       
        return img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid


    def __call__(self, img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid):
        ...
        img1_rain, img2_rain = self.color_transform(img1_rain, img2_rain)
        ...
        img1_rain, img2_rain = self.eraser_transform(img1_rain, img2_rain)
        img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid = self.spatial_transform(img1_clean, img2_clean, img1_foggy, img2_foggy, img1_rain, img2_rain, flow, valid)
        ...
        img1_rain = np.ascontiguousarray(img1_rain)
        img2_rain = np.ascontiguousarray(img2_rain)
        flow = np.ascontiguousarray(flow)
        valid = np.ascontiguousarray(valid)
